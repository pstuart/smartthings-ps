 ...
 */
metadata {
  definition (name: "Zen Thermostat New UI", namespace: "zenwithin", author: "ZenWithin") {
    capability "Actuator"
  ...
  tiles (scale:2) {
  		multiAttributeTile(name:"richtstat", type:"lighting", width:6, height:4) {
    		tileAttribute("device.temperature", key: "PRIMARY_CONTROL") {
      			attributeState "temperature", label: '${currentValue}', backgroundColor:"#0a1ec2"
    		}
            ...
            /*
            tileAttribute("statusText", key: "VALUE_CONTROL") {
      			attributeState "UpandDown", label: '${currentValue}'
    		}
            * Currently broken on android
            */
            
            /*
            tileAttribute("statusText", key: "SLIDER_CONTROL") {
        ...
        standardTile("setpointUp", "device.thermostatSetpoint", decoration: "flat", width: 3, height:1) {
            state "setpointUp", action:"setpointUp", icon:"st.thermostat.thermostat-up", backgroundColor:"#0a1ec2"
        }
        
        standardTile("setpointDown", "device.thermostatSetpoint", decoration: "flat", width: 3, height:1) {
            state "setpointDown", action:"setpointDown", icon:"st.thermostat.thermostat-down", backgroundColor:"#0a1ec2"
        }
      ...
      main "richtstat"
      details(["richtstat", "setpointUp", "setpointDown", "temperature", "fanMode", "mode", "thermostatSetpoint", "refresh", "configure"])
  }
}
...
def parse(String description) {
    ...
    //statusTextmsg = "This is a static message"
    sendEvent("name":"statusText", "value":statusTextmsg)
    def tstatLevel = device.currentState('thermostatSetpoint').value
    sendEvent("name":"level", "value":tstatLevel)
...
def setLevel(value) {	
    log.debug "Selected Level is $value"
    def privMin = 55
    def privMax = 85
    def newTempF = (((value.toInteger() - 1) * (privMax.toInteger() - privMin.toInteger()) / (100/1) + privMin.toInteger()))
   	log.debug "Adjusted Range value is $newTempF"
    setSetpoint(newTempF)
}